	...
	string get_connection_point_impl()
	{
		...
		if (!only_stub_iface())
		{
			for (MEMBERS::iterator it = members_.begin(); it != members_.end(); ++it)
			{
				try {
					if ((*it)->can_implement())
						os << (*it)->get_connection_point_impl("Fire_", "Fire_", "", "comet::cp_throw()", false, true) << std::endl;
				} CATCH_ALL("member '" <<(*it)->get_name() <<"'"  )
			}
//			for (MEMBERS::iterator it = members_.begin(); it != members_.end(); ++it)
//			{
...
string Type::get_impl_in_constructor(const string& r, bool asVariantType, bool ignore_pointer_level ) const {
	...
	if (vt & TLI::VT_ARRAY) {
		if (pointer_level > 1) throw exception("Invalid [in] type [VT_BSTR]. Reason: pointer_level > 1)");
		bool dummy;
		unsigned vsz = 0;
		s = get_cpp(0, vsz, dummy);
	...
	switch (vt) {
	case TLI::VT_BOOL:
		if (pointer_level > 0) throw exception("Invalid [in] type [VT_BOOL]. Reason: pointer_level > 0");
	...
	case TLI::VT_BSTR:
		...
		break;
	case TLI::VT_DISPATCH:
		if (pointer_level != 0) throw exception("Invalid [in] type [VT_DISPATCH]. Reason: pointer_level != 0");
		s = "com_ptr<IDispatch>::create_const_reference(";
		s += r;
		s += ")";
		break;
	case TLI::VT_VARIANT:
		if (pointer_level > 1) throw exception("Invalid [in] type [VT_VARIANT]. Reason: pointer_level > 1");
		...
		break;
	case TLI::VT_CY:
		if (pointer_level != 0) throw exception("Invalid [in] type [VT_CY]. Reason: pointer_level != 0");
		s = "currency_t(";
		s += r;
		s += ")";
		break;
	case TLI::VT_DATE:
		if (pointer_level != 0) throw exception("Invalid [in] type [VT_DATE]. Reason: pointer_level != 0");
		s = "datetime_t(";
		s += r;
		s += ")";
		break;
	...
	case TLI::VT_UNKNOWN:
		if (pointer_level != 0) throw exception("Invalid [in] type [VT_UNKNOWN]. Reason: pointer_level != 0");
	...
	case TLI::VT_EMPTY:
		{
			...
			if( !asVariantType)
			{
				...
				case TLI::TKIND_DISPATCH:
					...
					s += r;
					s += ")";
				if (!ignore_pointer_level && pointer_level != 1) throw exception("Invalid [in] type [VT_EMPTY]. Reason: pointer_level != 1");
					break;
				case TLI::TKIND_RECORD:
				{
					string unwrapped_type = nso.get_name_with_ns(get_unaliased_typeinfo(), true);
...
string Type::get_impl_inout_constructor(const string& r, bool asVariantType ) const {
	...
	if (vt & TLI::VT_ARRAY) {
		if (get_pointer_level() != 1) throw exception("Invalid [in, out] type [VT_BSTR]");
		bool dummy;
		unsigned vsz = 0;
		s = get_cpp(0, vsz, dummy);
	...
	switch (vt) {
	case TLI::VT_BOOL:
		if (get_pointer_level() != 1) throw exception("Invalid [in, out] type [VT_BOOL]");
	...
	case TLI::VT_BSTR:
		...
		break;
	case TLI::VT_DISPATCH:
		if (get_pointer_level() != 1) throw exception("Invalid [in, out] type [VT_DISPATCH]");
		s = "com_ptr<IDispatch>::create_reference(*";
		s += r;
		s += ")";
		break;
	case TLI::VT_VARIANT:
		if (get_pointer_level() != 1) throw exception("Invalid [in, out] type [VT_VARIANT]");
		s = "variant_t::create_reference(*";
		s += r;
		s += ")";
		break;
	case TLI::VT_CY:
		if (get_pointer_level() != 1) throw exception("Invalid [in, out] type [VT_CY]");
		s = "currency_t::create_reference(*";
		s += r;
		s += ")";
		break;
	case TLI::VT_DATE:
		if (get_pointer_level() != 1) throw exception("Invalid [in, out] type [VT_DATE]");
	...
	case TLI::VT_UNKNOWN:
		if (get_pointer_level() != 1) throw exception("Invalid [in, out] type [VT_UNKNOWN]");
	...
	case TLI::VT_EMPTY:
		{
			...
			if( !asVariantType)
			{
				...
				case TLI::TKIND_INTERFACE:
				case TLI::TKIND_DISPATCH:
					if (get_pointer_level() != 2) throw exception("Invalid [in, out] type [VT_EMPTY]");
					s = "com_ptr<";
					s += nso.get_name_with_ns(ti);
					s += ">::create_reference(*";
					s += r;
				...
				case TLI::TKIND_RECORD:
				{
					...
					if ( unwrapped_type == "::GUID")
					{
						if (get_pointer_level() != 1) throw exception("Invalid [in, out] type [VT_EMPTY, GUID]");
					...
					{
						if (get_pointer_level() != 1) throw exception("Invalid [in, out] type [VT_EMPTY, TKIND_RECORD]");
			...
			else
			{
				switch (tk )
				{
					...
					case TLI::TKIND_DISPATCH:
						...
						s += r;
						s += ")";
					if (get_pointer_level() != 1) throw exception("Invalid [in, out] type [VT_EMPTY]");
						break;
					default:
						return "*" + r;
				}
